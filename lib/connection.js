...
module.exports = (function() {
	 ...
	 *           certificate. See the options for node tls.connect for more info.
	...
	function Connection(host, user, password, options) {
		 ...
		 * @memberof mikronode.Connection
		 */
		utils.createProperty(this, 'port', options.port || (options.tls ? 8729 : 8728));
		 ...
		 * @memberof mikronode.Connection
		 */
		utils.createProperty(this, 'tls', options.tls);
		 ...
		 * @memberof mikronode.Connection
		 */
		utils.createProperty(this, 'channels', {}, false, _);
	...
	Connection.prototype.connect = function connect(callBack) {
		...
		if (this.tls) {
			var tlsopts = ((typeof this.tls === 'boolean') ? {} : this.tls);
			_(this).socket = new tls.TLSSocket(tempSocket, tlsopts);
		} else {
			_(this).socket = tempSocket;
		}

		debugSocket('Created%s socket to %s:%d', this.tls ? ' TLS' : '', this.host, this.port);
		 ...
		 */
		if (this.timeout) {
			debugConnection('Setting timeout to %d seconds', this.timeout);
			/**
			 * Calling setTimeout with a callback is safer than socket.on('timeout') for TLS
			 * sockets
			 */
			_(this).socket.setTimeout(this.timeout * 1000, function() {
				debugSocket('Timeout');
		 ...
		 */
		tempSocket.connect(this.port, this.host, function lsm() {
	...
	Connection.prototype.loginStateMachine = function loginStateMachine() {
		...
		lc.on('trap', function loginTrap(data) {
			...
			_(this.connection).status = 'Received Trap';
			debugLogin('%s %o', _(this.connection).status, data);
			this.connection.emit('trap', data);
			this.close(true);
		...
		lc.on('done', function loginDone(data) {
			...
			switch (_(this.connection).status) {

			case 'Waiting for challenge':
				_(this.connection).status = 'Received challenge';
				debugLogin('%s %o', _(this.connection).status, parsed);
				...
				_(this.connection).status = 'Sending Credentials';
				debugLogin('%s %s', _(this.connection).status, this.connection.user);
				this.write('/login', {
					"=name" : this.connection.user,
					"=response" : "00"
							+ crypto.createHash('md5').update(emptyString + this.connection.password + challenge)
									.digest("hex")
				});
				_(this.connection).status = 'Waiting for Response';
				debugLogin(_(this.connection).status);
				return;

			case 'Waiting for Response':
				_(this.connection).status = 'Received Response';
				debugLogin('%s %o', _(this.connection).status, parsed);
				this.close(true);
				_(this.connection).status = 'Connected';
				debugLogin(_(this.connection).status);
				_(this.connection).connected = true;

				if (_(this.connection).connectionCallback) {
					_(this.connection).connectionCallback(this.connection);
					_(this.connection).connectionCallback = null;
				}

				this.connection.closeOnDone = tempcOD;
			}
		});
	};
	...
	Connection.prototype.closeChannel = function closeChannel(id) {
		...
		if (Object.keys(_(this).channels).length === 0 && ((_(this).closing || this.closeOnDone))) {
			this.close();
		}
	};
