...
module.exports = (function() {
	...
	function login(conn) {
		conn.addListener('sentence', conn.sentence);
		var tempcOD = conn.closeOnDone;
		conn.closeOnDone = false;
		_(this).currentReply = 'login';

		_(conn).socket.connect(conn.port, conn.host, function() {
			var lc = conn.openChannel('login');
			lc.closeOnDone = false;

			lc.on('trap', function(data) {
				lc.close(true);
				conn.closeOnDone = tempcOD;
				conn.emit('trap', data);
			});

			lc.on('done', function(data) {
				debugLogin('DONE: %o', data);
				...
				case 'Sending Login':
					...
					if (challenge.length !== 16) {
						...
						conn.removeListener('sentence', conn.loginHandler);
						conn.close();
					} else {
						lc.write('/login', {
							"=name" : conn.user,
