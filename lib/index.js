...
(function() {
  ...
  api.prototype.sentence = function(data, more) {
    if (this.debug > 2) console.error('Sentence:(' + more + ') data: ' + data);
    if (this.fatal) { // our last message was a fatal error.
      //console.error('Sentence: fatal error: '+data);
    ...
    if (data == '!done') { // we got a done signal... but we could be in a channel.
      ...
      this.buffer = [];
      if (this.debug > 2) console.error('Sentence: Done Signal.');
      if (this.trap) { // we previously caught a trap
        this.trap = false;
        var e = this.error;
        this.error = {};
        if (this.debug > 2) console.error('Sentence: Sending trap.');
        this.handler.emit('trap', e, this.handler);
      } else { // no trap. Send general packet.
        if (!more) {
          if (this.debug > 2) console.error('Sentence: No more data in packet. Done.');
          this.handler.emit('done', this.packet);
        } else {
          if (this.debug > 2) console.error('Sentence: Could have a tag.');
          this.nextTag = 1;
        }
      }
      //else
    ...
    if (/=ret=/.test(data)) {
      if (this.debug > 2) console.error('Sentence: Single return: ' + data);
      this.buffer.push('!re');
    ...
    if (this.nextTag) { // We had a done event, this could be a tag.
      ...
      if (data.match(/\.tag/)) { // Check if we have a tag.
        ...
        if (this.debug > 2) console.error('Sentence: Done channel ' + channel + '.');
    ...
    if (data.match(/\.tag/)) { // Catch tags where it's not following !done
      ...
      var channel = data.substring(5);
      if (this.trap) { // we previously caught a trap
        if (!this.channel[channel]) return console.error('ERROR: No channel for trap');;
        this.trap = false; // we're emitting the trap. Clear it.
        var e = this.error;
        this.error = {};
        if (this.channel[channel]) {
          if (this.debug > 2) console.error('Sentence: Sending trap in channel: ' + channel + " data:" + JSON.stringify(e));
          this.channel[channel].emit('trap', e, this.channel[channel]);
        }
      } else // no trap. Send general packet.
      if (this.channel[channel])
    ...
    if (this.trap) {
      ...
      if (l.length > 1) {
        this.error[l[1]] = l[2];
        if (this.debug > 2) console.error('Sentence: Trap property: ' + l[1] + ' = ' + l[2]);
      }
    } else
    if (data.match(/\!trap/)) {
      this.trap = true;
      if (this.debug > 2) console.error('Sentence: catching trap');
    } else {
  ...
  api.prototype.read = function(data) { // This is the handler for when the socket receives data. Don't call instance function directly
    if (this.debug > 4) {
      hexDump(data);
      //console.error('read: new packet:'+);
    }
    while (data.length) {
      if (this.debug > 3) console.error('read: data-len:' + data.length);
      if (this.len) { // maintain the current data length. What if the data comes in 2 separate packets?
        ...
        //if (this.debug>3) console.error('read: data:'+data);
        if (data.length <= this.len) {
          this.len -= data.length;
          this.line += data.toString();
          if (this.debug > 3) console.error('read:consume-all: data:' + data);
          if (this.len == 0) {
          ...
          if (this.debug > 3) console.error('read:consume len:(' + this.len + ') data: ' + data);
  ...
  api.prototype.write = function(a) { // This shouldn't be called directly. Please use channels.
    if (!this.connected && !this.connecting) {
      if (this.debug > 2)
        console.error('write: not connected ');
      return;
    }
  ...
  function Connection(instance) {
    var o = {
      ...
      closeOnFatal: function(b) {
        if (typeof(b) === 'boolean')
          if (!instance.closeOnFatal && b) instance.closeOnFatal = b ? self.addListener('fatal', function(conn) {
            instance.close()
          }) : false;
      ...
      setHost: function(h) {
        if (instance.connected) return this;
        this.host = h;
        return this
      },
      connect: function(callBack) {
        ...
        loginHandler = function(d) {
          if (instance.debug > 2)
            console.error('LoginHandler: ' + self.status + ' : ' + self.getHost());
          switch (self.status) {
            case 'Connecting':
              self.status = 'Sending Login';
              if (instance.debug > 2) console.error(self.status);
              instance.write('/login');
              break;
            case 'Sending Login':
              if (d.length < 1) return;
              if (d == '!done') {
                if (instance.debug > 2) console.error('Got !done. Need challenge');
              ...
              if (/=ret=/.test(d)) {
                ...
                if (challenge.length != 16) {
                  ...
                  if (instance.debug > 3) console.error('Challenge length:' + challenge.length);
                  if (instance.debug) console.error(instance.error);
            ...
            case 'Sending Credentials':
              ...
              if (d == '!done') {
                ...
                instance.addListener('sentence', function(data, more) {
                  instance.sentence(data, more)
                });
                if (instance.debug > 2)
                  console.error(self.status);
        ...
        instance.addListener('fatal', function(conn) {
          self.close()
        });
        instance.addListener('error', function(e) {
          self.emit('error', e, self)
        });
      ...
      openChannel: function(id) {
        ...
        if (instance.debug > 0)
          console.error('Opening channel: ' + id);
        var o = instance.channel[id] = new apiChannel(id, this);
        var self = this;
        o.addListener('close', function() {
          if ((instance.closing || instance.closeOnDone) && Object.keys(instance.channel).length) self.close();
        });
        return o;
      },
      getChannel: function(id) {
        ...
        if (instance.debug > 0)
          console.error('Getting channel: ' + id);
        return instance.channel[id];
      },
      write: function(a) {
        return instance.write(a)
      },
      closeChannel: function(id) {
        ...
        if (instance.debug > 0)
          console.error('Closing ' + this.getHost() + ' channel: ' + id);
      ...
      close: function(force) {
        ...
        var self = this;
        if (force) Object.keys(instance.channel).forEach(function(e) {
          self.channel[e].close(true);
        });
  ...
  function Channel(instance) {
    ...
    var o = {
      ...
      closeOnDone: function(b) {
        return typeof(b) == 'boolean' ? instance.closeOnDone = b : instance.closeOnDone;
      },
      clearEvents: function(b) {
        return typeof(b) == 'boolean' ? instance.clearEvents = b : instance.clearEvents;
      },
      write: function(d, writeCallback) {
        ...
        if (d) {
          ...
          if (this.getConnection().debug > 0) console.error('Channel (' + this.getConnection().getHost() + ":" + this.getId() + ') write: empty arg.');
        }
        if (this.running()) {
          if (this.getConnection().debug > 0) console.error('Channel (' + this.getConnection().getHost() + ":" + this.getId() + ') write: pushing command.');
          ...
          this.callback = null;
          if (this.getConnection().debug > 0) console.error('Channel (' + this.getConnection().getHost() + ":" + this.getId() + ') write lines: ' + JSON.stringify(b));
        ...
        if (this.getConnection().debug > 0)
          console.error('Channel done: (' + this.getConnection().getHost() + ":" + this.getId() + ')');
        ...
        if (instance.commands.length) {
          ...
          if (this.getConnection().debug > 0)
            console.error('Channel (' + this.getConnection().getHost() + ":" + this.getId() + ') buffered commands(' + (instance.commands.length + 1) + '): ' + JSON.stringify(c));
          cl = instance.closing;
          instance.closing = false;
          this.write(c[0], c[1]);
          instance.closing = cl;
      ...
      data: function(data) { // Don't call instance directly. This is called when a full sentence is received (typically a !re line)
        if (this.getConnection().debug > 2)
          console.error('Channel data: ' + data);
        if (this.saveBuffer())
      ...
      close: function(force) { // Close instance channel.
        ...
        if (this.getConnection().debug > 1)
          console.error('Closing host:channel: ' + this.getConnection().getHost() + ':' + this.getId());
        this.closed(true);
        this.getConnection().closeChannel(this.getId());
        this.emit('close', this);
      },
      finalize: function() {
        if (this.getConnection().debug > 3) console.error('Channel Finalize: ' + this.getId());
